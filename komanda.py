...
def calculate_tab(comanda):
    element_Calc = 0
    for cont in range(len(comanda)):
        for cont2 in range(len(menu)):
            itemComanda= comanda[cont]
            itemMenu= menu[cont2]
            if itemComanda["id"]== itemMenu["id"] :
                element_Calc+= itemMenu["price"]*itemComanda["amount"]
    return  element_Calc
